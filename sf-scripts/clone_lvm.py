...
# jtw adds for I-Behavior
    ...
    parser.add_option('-v', '--volume-group', action='store',
                      type='int',
                      default=None,
                      dest='volume_group',
                      help='Linux volume group name to clone')
...
if __name__ == "__main__":
    ...
    volid_list = []
    # jtw for volume group clones
    # For this we figure out the physical volumes (PV) for a Linux
    # LVM by using the sdparm -i command and parse the output to
    # get the SolidFire volume ID for each of the physical volumes
    # in the volume group.  Once we have the SolidFire volume IDs
    # we use dmsetup suspend to suspend the LVM, then clone, then
    # run dmsetup resume.
    if options.volume_group is not None:
        p1 = Popen( [ "vgdisplay -v", stdout = PIPE ] )
        p2= Popen( [ "grep", options.volume_group ], stdin=p1.stdout, stdout=PIPE)
        p3= Popen( [ "awk '{print $NF", options.volume_group ], stdin=p1.stdout, stdout=PIPE)
    ...
    for v in vol_list:
        ...
        if v['volumeID'] in volid_list:
            ...
            name = v['name']
# jtw begin mods for I-Behavior
            clone_name = ("%s-clone-%s" % (name, options.mart_name ) )
# jtw end mods for I-Behavior

            logging.info('Cloning volume:%s...' % id)
            for x in range(0, options.clones_per_volume):
                # jtw clone_name = ("%s-clone-%s" % (name, x))
                # jtw begin mods for I-Behavior
                clone_name = ("%s-clone-%s-%s" % (name, x, options.mart_name ) )
                # jtw end mods for I-Behavior
                if options.target_account_id == 0:
                    params = {'volumeID': int(id),
                              'name': clone_name}
                else:
                    params = {'volumeID': int(id),
                              'name': clone_name,
                              'newAccountID': options.target_account_id}

                logging.info('Creating clone %s of Volume %s:' % (x + 1, id))

                try:
                    async_handle = SFC.issue_api_request('CloneVolume', params)
