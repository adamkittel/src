...
def process_options():
    ...
    # jtw
    parser.add_option('-n', '--name-wildcard', action='store',
                      type='string',
                      default="",
                      dest='name_wildcard',
                      help='Name wildcard to match'
                      ' ie: \"clone-mart-a"',)
    # jtw
...
if __name__ == "__main__":
    ...
    for v in vol_list:
        if v['volumeID'] in volid_list:
            if v['name'].find( options.name_wildcard ) != -1:
               # logging.info('VolumeID:%d:VolumeName:%s\n' % (v['volumeID'], v['name']) )
               print "%s" % str(v['volumeID'] )
               output_list += str(v['volumeID']) 
               # jtw logging.info('Volume:%s\n' % v)
               counter += 1
               if counter < len(vol_list):
                  output_list += ","
    ...
    if counter < len(volid_list):
        logging.info('Only displayed %s of %s requested'\
