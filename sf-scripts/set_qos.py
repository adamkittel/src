...
if __name__ == "__main__":
    ...
    # jtw begin mod
                ...
                'default': {'minIOPS': 100,
                            'maxIOPS': 15000,
                            'burstIOPS': 18750},
                'ora-reco': {'minIOPS': 1000,
                             'maxIOPS': 100000,
                             'burstIOPS': 100000},
                'ora-data': {'minIOPS': 15000,
                             'maxIOPS': 100000,
                             'burstIOPS': 100000},
    # jtw end mod
                'off': None}
    ...
    for v in vol_list:
        ...
        if v['volumeID'] in volid_list:
            params = {'volumeID': int(v['volumeID']),
                      ...
                      'qos': qos_dict[options.qos_level]}

            logging.info('Setting QOS on volumeID:%s' % id)
            logging.debug('QOS settings are:%s' % qos_dict[options.qos_level])
            result = SFC.issue_api_request('ModifyVolume', params)
            if 'error' in result:
                logging.error('Failed to set QOS on volume:%s' %
                        result['error'])
            else:
                counter += 1
