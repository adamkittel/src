...
for DS in `/usr/bin/perl ${VMWBIN}/vmware_list_datastores.pl --cluster=${VMW_CLUSTER} --bash --mgmt_server=${VCENTER}`
	...
	do
		...
		/usr/bin/perl ${VMWBIN}/vmware_clone_vm.pl --clone_name=ubuntuServer${NUM} --datastore=${DS} --mgmt_server=${VCENTER} --nopoweron --source_vm=ubuntuServer --thin --vmhost=${HOST} & 
		sleep 5
		...
		#	echo "FAIL. ${UBUNTU}"
		#fi
		#WIN7=`/usr/bin/perl ${VMWBIN}/vmware_clone_vm.pl --clone_name=Windows7${x} --datastore=${DS} --mgmt_server=${VCENTER} --nopoweron --source_vm=Windows7 --thin --vmhost=${HOST} &`
		...
		#	echo "FAIL. ${WIN7}"
		#fi
		#ADSVR=`/usr/bin/perl ${VMWBIN}/vmware_clone_vm.pl --clone_name=ADserver${x} --datastore=${DS} --mgmt_server=${VCENTER} --nopoweron --source_vm=ADserver --thin --vmhost=${HOST} &`
