...
namespace SfAutomation
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            else if (Args["login_targets"] != null)
            {
                ...
                try
                {
                    ...
                    if (logged_in > 0)
                    {
                        Logger.Info("Onlining/signaturing all SolidFire disks");
                        msinit.OnlineDisks();
                    }
            ...
            else if (Args["list_disks"] != null)
            {
                ...
                foreach (MicrosoftInitiator.DiskInfoDetailed disk in disk_list)
                {
                    ...
                    {
                        output += " (FC)" + ", volumeID: " + disk.VolumeID;;
                    }
                    output += 
            ...
            else if (Args["online_disks"] != null)
            {
                ...
                if ((arg_device_list != null && arg_device_list.Count > 0) || (arg_target_iqns != null && arg_target_iqns.Count > 0))
                    ...
                    statement += " all";
                statement += " SolidFire disk devices";
                if (arg_portal_list != null && arg_portal_list.Count > 0)
                    statement += " on portals [" + String.Join(",", arg_portal_list.ToArray()) + "]";
                Logger.Info(statement);
            ...
            else if (Args["enable_mpio"] != null)
            {
                ...
                try
                {
                    reboot_required = msinit.EnableMpio();
                }
                catch (MicrosoftInitiator.InitiatorException e)
                {
            ...
            else if (Args["disable_mpio"] != null)
            {
                ...
                try
                {
                    reboot_required = msinit.DisableMpio();
                }
                catch (MicrosoftInitiator.InitiatorException e)
                {
            ...
            else if (Args["disk_map"] != null)
            {
                Dictionary<int, string> disk_map = new Dictionary<int, string>();
                try
                {
                    disk_map = msinit.GetDiskMap(PortalAddressList: arg_portal_list, TargetList: arg_target_iqns);
                }
                catch (MicrosoftInitiator.InitiatorException e)
                {
                    Logger.Error("Listing disks failed: " + e.Message);
                ...
                foreach (KeyValuePair<int, string> item in disk_map)
                {
                    Logger.Info("VolumeID: " + item.Key + ", Device: " + item.Value);
                }
                if (json)
                    Console.WriteLine(Newtonsoft.Json.JsonConvert.SerializeObject(disk_map, Newtonsoft.Json.Formatting.Indented));
            }
            else if (Args["path_map"] != null)
            {
                Dictionary<int, int> path_map = new Dictionary<int, int>();
                try
                {
                    path_map = msinit.GetPathMap();
                }
                catch (MicrosoftInitiator.InitiatorException e)
                {
                    Logger.Error("Listing paths failed: " + e.Message);
                    LogExceptionDetail(e);
                    if (batch || json)
                        Console.Error.WriteLine("Listing paths failed: " + e.Message);
                    Environment.Exit(EXIT_FAIL);
                }
                foreach (KeyValuePair<int, int> item in path_map)
                {
                    Logger.Info("VolumeID: " + item.Key + ", HealthyPathCount: " + item.Value);
                }
                if (json)
                    Console.WriteLine(Newtonsoft.Json.JsonConvert.SerializeObject(path_map, Newtonsoft.Json.Formatting.Indented));
            }
        ...
        static void PrintUsage()
        {
            ...
            Console.WriteLine("  --enable_mpio         Enable MPIO and add the SF device string so that SF FC");
            Console.WriteLine("                        volumes are claimed by the MS DSM (usually requires");
            Console.WriteLine("                        a reboot)");
            Console.WriteLine("  --disable_mpio        Disable MPIO (usually requires a reboot)");
            ...
            Console.WriteLine("  --list_paths          Display the list of SolidFire volumes and their paths.");
            ...
            Console.WriteLine("  --online_disks        Set SolidFire disk devices online and read-write.");
            ...
            Console.WriteLine("  --disk_map            Display simple list of devices and their SF volume IDs");
            Console.WriteLine("  --path_map            Display simple list of devices and their count of ");
            Console.WriteLine("                        healthy paths");
            Console.WriteLine();
            //                 0        1         2         3         4         5         6         7         8
            //                 12345678901234567890123456789012345678901234567890123456789012345678901234567890
            ...
            Console.WriteLine("                        the name of the SolidFire volume");
