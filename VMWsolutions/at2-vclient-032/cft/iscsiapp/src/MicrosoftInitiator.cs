...
namespace SfAutomation
{
    class MicrosoftInitiator
    {
        ...
        private Dictionary<string, string> GetDeviceToIscsiVolumeMap(List<string> PortalAddressList = null, List<string> TargetList = null)
        {
            ...
            Dictionary<string, string> device_to_sfvolume = new Dictionary<string, string>();
            foreach (ManagementObject session in session_list)
            {
                ...
                if (!String.IsNullOrEmpty(target_name) && targets_to_query.Contains(target_name))
                {
                    ...
                    // Name for the volume, based on SF volume name and ID
                    string[] pieces = target_name.Split('.');
                    string volume_name = pieces[4] + "." + pieces[5];

                    device_to_sfvolume.Add(device_name, volume_name);
                }
            }

            return device_to_sfvolume;
        }
        ...
        /// <summary>
        /// Remove volume mount points from SF disk devices
        /// </summary>
        ...
        public void RemoveMountpoints(List<string> DeviceList = null, List<string> PortalAddressList = null, List<string> TargetList = null)
        {
            ...
            foreach (Pack disk_pack in vds_provider.Packs)
            {
                ...
                // Make sure this pack contains SF disks that meet the filter criteria
                foreach (AdvancedDisk disk in disk_pack.Disks)
                {
                    ...
                    if (!disk.DevicePath.ToLower().Contains("solidfir"))
                        break;
        ...
        /// <summary>
        /// Unmount volumes and take SF disk devices offline
        /// </summary>
        /// <param name="DeviceList">Only operate on these devices</param>
        /// <param name="PortalAddress">Only operate on devices from iSCSI targets on this portal</param>
        /// <param name="TargetList">Only operate on devices from these iSCSI targets</param>
        ...
        public void UnmountAndOfflineDisks(List<string> DeviceList = null, List<string> PortalAddressList = null, List<string> TargetList = null, bool ForceUnmount = false)
        {
            ...
            // Go through disk packs and find SF disks
            foreach (Pack disk_pack in vds_provider.Packs)
            {
                string dev_name = "";
                bool skip = false;
        ...
        /// <summary>
        /// Bring SF disk devices online and RW
        /// </summary>
        /// <param name="DeviceList">Only operate on these devices</param>
        ...
        /// <summary>
        /// Create partitions and format SF disk devices
        /// </summary>
        /// <param name="DeviceList">Only operate on these devices</param>
        /// <param name="PortalAddressList">Only operate on devices from iSCSI targets on these portals</param>
        /// <param name="TargetList">Only operate on devices from these iSCSI targets</param>
        ...
        public void PartitionAndFormatDisks(List<string> DeviceList = null, List<string> PortalAddressList = null, List<string> TargetList = null, bool RelabelVolumes = false)
        {
            ...
            // Find all SolidFire disks in all disk packs and make sure they are partitioned, formatted and mounted
            foreach (Pack disk_pack in vds_provider.Packs)
            {
                bool sf_disks = false;
                string volume_label = "";
                ...
                foreach (Volume vol in disk_pack.Volumes)
                {
                    ...
                    if (RelabelVolumes)
                    {
                        // Verify that the volume label is the same as the SF volume name, and relabel as necessary
        ...
        /// <summary>
        /// Create mountpoints for SF disk devices
        /// </summary>
        /// <param name="DeviceList">Only operate on these devices</param>
        /// <param name="PortalAddressList">Only operate on devices from iSCSI targets on these portals</param>
        /// <param name="TargetList">Only operate on devices from these iSCSI targets</param>
        ...
        private void WaitForDiskDevices()
        {
            ...
            while (true)
            {
                ...
                foreach (SoftwareProvider prov in provider_list)
                {
                    foreach (AdvancedDisk disk in vds_service.UnallocatedDisks)
                    {
                        // We used to look for SolidFire disks by matching the Vendor string, like this: if (disk.FriendlyName.Contains("SolidFir"))
                        // However, it looks like VDS randomly returns an empty string for the FriendlyName if you have more than a handful of volumes, so this is not reliable.
        ...
        /// <summary>
        /// Get a list of the SolidFire disk devices connected to the system
        /// </summary>
        ...
        public List<DiskInfoDetailed> ListDiskInfo(List<string> PortalAddressList = null, List<string> TargetList = null)
        {
            ...
            while (true)
            {
                ...
                foreach (var disk in wmi_disk_list)
                {
                    // The old way was to blacklist certain model strings
                    //// Check for VMware/iDRAC disks and skip
                    //string model = disk["Model"] as String;
                    //if (model == null)
                    //{
                    //    Logger.Debug(disk["DeviceID"] + " has a null model");
                    //}
                    //if (model != null && (model.ToLower().Contains("vmware") || model.ToLower().Contains("idrac")))
                    //    continue;

                    // The new way is to whitelist SolidFir disks.  This means we now depend on the model string being present and non-null
                    string model = disk["Model"] as String;
                    if (String.IsNullOrEmpty(model))
                    {
                        Logger.Debug("Disk " + disk["DeviceID"] + " has a null model string");
                        if (show_warning)
                        {
                    ...
                    // Skip this drive if it is not a SolidFire disk
                    if (!model.ToLower().Contains("solidfir"))
                        continue;

                    string sernum = disk["SerialNumber"] as String;
                    if (sernum == null)
                    {
                        Logger.Debug("Disk " + disk["DeviceID"] + " has a null serial number");
                        if (show_warning)
                        {
                            Logger.Warn("Detected null disk info; waiting for disk database to be up to date");
                            show_warning = false;
            ...
            foreach (var disk_info in disks_to_return)
            {
                ...
                if (devid2serial.ContainsKey(dev_id))
                {
                    ...
                    for (int i = 0; i < 8; i += 2)
                    {
                        disk_info.ClusterID += Convert.ToChar(int.Parse(disk_info.EUISerialNumber.Substring(i, 2), System.Globalization.NumberStyles.AllowHexSpecifier));
                    }
                    disk_info.VolumeID = int.Parse(disk_info.EUISerialNumber.Substring(8, 8), System.Globalization.NumberStyles.AllowHexSpecifier);
                }
            }
        ...
        /// <summary>
        /// Map a VDS disk object to a solidfire DiskInfo object.  Returns null of the disk is not a SolidFire device,
        ...
        private DiskInfoDetailed VdsDiskToDiskInfo(Disk VdsDisk, List<IscsiSessionInfo> IscsiSessions, HashSet<string> MatchIscsiTargets = null)
        {
            ...
            // Skip this disk if it is not a SolidFire device
            // VDS sometimes gets into a state where the FriendlyName it returns is an empty string, so we check the device path as well
            if (!VdsDisk.FriendlyName.ToLower().Contains("solidfir") && ! VdsDisk.DevicePath.ToLower().Contains("solidfir"))
                return null;
        ...
        /// <summary>
        /// Get a list of all the SolidFire disks and their path info
        /// </summary>
        /// <returns></returns>
        public List<MpioDiskInfoDetailed> ListMpioDiskInfo()
        {
            ...
            foreach (var disk in dsm_disk_list)
            {
                ...
                if (instance_name.ToLower().Contains("solidfir"))
                {
        ...
        /// <summary>
        /// Enable the MPIO feature, and add the SF device string to the MS DSM
        /// The return value indicates if a reboot is required
        /// </summary>
        /// <returns></returns>
        public bool EnableMpio()
        {
            Logger.Info("Enabling MPIO for SolidFire devices");
            ...
            //
            // Make sure the SF devices are added to the MS DSM
            //
            string SF_DEVICE_STRING = "SolidFirSSD SAN         ";
            bool sf_registered = false;

            // Check if the SF device string is already claimed by MPIO
            cmd = "mpclaim.exe -h";
            res = RunCommand(cmd);
            if (res.ExitCode != 0)
            {
                throw new InitiatorException("Failed to query for SF device string - " + res.Stdout + res.Stderr);
            }
            foreach (string line in Regex.Split(res.Stdout, "\n"))
            {
                if (line.Contains(SF_DEVICE_STRING))
                {
                    sf_registered = true;
                    break;
                }
            ...
            // Add the SF device string to MPIO/MS DSM if it isn't already
            if (!sf_registered)
            {
                reboot_required = true;
                cmd = "mpclaim.exe -n -i -d \"" + SF_DEVICE_STRING + "\"";
                res = RunCommand(cmd);
                if (res.ExitCode != 0)
                {
                    throw new InitiatorException("Failed to register SF device string - " + res.Stdout + res.Stderr);
                }
            }
            ...
            //// add SF devices to MPIO
            //string path = @"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\MPDEV";
            //string key = "MPIOSupportedDeviceList";
            //List<string> device_list = new List<string>(
            //    Microsoft.Win32.Registry.GetValue(path, key, new string[]{}) as string[]
            //);
            //if (!device_list.Contains(SF_DEVICE_STRING))
            //{
            //    device_list.Add(SF_DEVICE_STRING);
            //    Microsoft.Win32.Registry.SetValue(path, key, device_list.ToArray());
            //    reboot_required = true;
            //}

            //// add SF devices to the MS DSM
            //path = @"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\msdsm\Parameters";
            //key = "DsmSupportedDeviceList";
        ...
        /// <summary>
        /// Remove the SF device string from the MS DSM
        /// The return value indicates if a reboot is required
        /// </summary>
        /// <returns></returns>
        public bool DisableMpio()
        {
            Logger.Info("Disabling MPIO for SolidFire devices");
            bool reboot_required = false;
            ...
            // Check if the SF device string is claimed by MPIO
            ...
            // Remove the SF device string from MPIO/MS DSM if it isn't already
            if (sf_registered)
            {
                reboot_required = true;
                cmd = "mpclaim.exe -n -u -d \"" + SF_DEVICE_STRING + "\"";
                res = RunCommand(cmd);
                if (res.ExitCode != 0)
                {
                    throw new InitiatorException("Failed to unregister SF device string - " + res.Stdout + res.Stderr);
                }
            }
        ...
        /// <summary>
        /// Get the map of SF volume ID => OS device name for automation use
        /// </summary>
        /// <param name="PortalAddressList">Only return iSCSI disks from these portals</param>
        /// <param name="TargetList">Only return iSCSI disks from these targets</param>
        /// <returns></returns>
        public Dictionary<int, string> GetDiskMap(List<string> PortalAddressList = null, List<string> TargetList = null)
        {
            Dictionary<int, string> disk_map = new Dictionary<int, string>();
            List<DiskInfoDetailed> disk_list = ListDiskInfo(PortalAddressList, TargetList);
            foreach( DiskInfoDetailed disk in disk_list)
            {
                disk_map.Add(disk.VolumeID, disk.LegacyDeviceName);
            }
            return disk_map;
        }

        /// <summary>
        /// Get the map of SF volume ID => healthy path count for automation use
        /// </summary>
        /// <returns></returns>
        public Dictionary<int, int> GetPathMap()
        {
            Dictionary<int, int> path_map = new Dictionary<int, int>();
            List<MpioDiskInfoDetailed> path_list = ListMpioDiskInfo();
            foreach (MpioDiskInfoDetailed disk in path_list)
            {
                path_map.Add(disk.VolumeID, disk.DSM_Paths.Count - disk.FailedPathCount);
            }
            return path_map;
        }
