...
# Special case if you want to reboot all but one node, set this to a valid IP
...
while [ 1 ]; do
    ...
    # Pick a random node to reboot
    loginfo "============ Selecting a random node in the cluster ============"
    NODE_IP=`python get_random_node.py --mvip=$SFMVIP` || fail "Failed picking a random node"
    ...
    if [ ! -z "$SFCLIENT_IPS" ]; then
        ...
        python check_client_health.py --client_ips=$SFCLIENT_IPS
    fi
