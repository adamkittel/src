# This test will power off and wait for syncing on each the nodes in the cluster one at a time
...
# Uses root/calvin as the credentials for IPMI - to use different credentials, set SFIPMI_USER and SFIPMIPASS env variables
...
# Special case if you want to fail all but one node, set this to a valid IP
...
loginfo "============ Getting a list of IPMI IP addresses ============"
DRAC_LIST=()
for ip in ${NODE_LIST[@]}; do
    DRAC_IP=`python get_node_ipmi_ip.py --node_ip=$ip --csv` || fail "Failed getting IPMI IP for $ip"
    DRAC_LIST[${#DRAC_LIST[*]}]=$DRAC_IP
done

# Make sure there are no faults before starting the test
loginfo "============ Making sure cluster is healthy ============"
...
while true; do
    ...
    for (( i=0; i<$NODE_COUNT; i++ )); do
        ...
        # Get a count of the expected number of drives in the node
        loginfo "============ Getting a count of the drives in $NODE_IP ============"
        DRIVE_COUNT=`python get_node_drive_count.py --mvip=$SFMVIP --node_ip=$NODE_IP --csv` || fail "Failed getting drive count from $NODE_IP"

        # Power off the node
        loginfo "============ Powering off $NODE_IP ($CURRENT_NODE / $NODE_COUNT) ============"
        python power_off_node.py --node_ip=$NOIDE_IP --ipmi_ip=$DRAC_IP || fail "Failed powering of node $NODE_IP"

        # Wait for the services to be decommissioned and synced out of the cluster
        loginfo "============ Waiting for 10 minutes ============"
        sleep 600

        # Wait for the services to finish syncing out of the cluster
        ...
        loginfo "============ Powering on $NODE_IP ============"
        python power_on_node.py --node_ip=$NOIDE_IP --ipmi_ip=$DRAC_IP || fail "Failed powering of node $NODE_IP"

        # Wait for the node to be fully up and drives available
        loginfo "============ Waiting for $DRIVE_COUNT drives to be available ============"
        python wait_for_available_drives.py --mvip=$SFMVIP --drive_count=$DRIVE_COUNT || fail "Failed waiting for drives to be available from $NODE_IP"
