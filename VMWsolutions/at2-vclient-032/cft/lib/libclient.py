...
class SfClient:
    ...
    def Connect(self, pClientIp, pUsername, pPassword):
        ...
        try:
            ...
            if (retcode == 0):
                ...
                # Make sure the latest version of iscsiapp.exe is on the client
                self._debug("Updating iscsiapp")
                if self.LocalOs == OsType.Windows:
                    lib_path = os.path.dirname(os.path.realpath(__file__))
                    command = "robocopy " + lib_path + r"\iscsiapp\bin \\" + str(self.IpAddress) + r"\c$\Windows\System32 iscsiapp.* /z /r:1 /w:3 /tbd"
                    retcode, stdout, stderr = libsf.RunCommand(command)
                    if retcode != 0:
                        raise ClientError("Failed to update iscsiapp on client: " + stderr)
                else:
                    command = "smbclient //" + self.IpAddress + "/c$ " + self.Password + " -U " + self.Username + " <<EOC\ncd Windows\\System32\nlcd " + lib_path + "/iscsiapp/bin\nput iscsiapp.exe\nexit\nEOC"
                    retcode, stdout, stderr = libsf.RunCommand(command)
                    if retcode != 0:
                        raise ClientError("Failed to update iscsiapp on client: " + stderr)
                    for line in stdout.split("\n"):
                        if line.startswith("NT_"):
                            raise ClientError("Failed to update iscsiapp on client: " + line)
    ...
    def UpdateInitiatorName(self):
        ...
        if self.RemoteOs == OsType.Windows:
            ...
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --show_initiatorname")
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
            ...
            for line in stdout.split("\n"):
                ...
                if m:
                    ...
                    break
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --set_initiatorname --name=" + initiator_name)
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
            return True
    ...
    def CleanIscsi(self, default_iscsid=True):
        if self.RemoteOs == OsType.Windows:
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --clean")
            if retcode == 0:
    ...
    def SetupChap(self, pPortalAddress, pChapUser, pChapSecret):
        ...
        if self.RemoteOs == OsType.Windows:
            ...
            cmd = "iscsiapp.exe --add_portal --portal_address=" + pPortalAddress + " --chap_user=" + pChapUser + " --chap_secret=\"" + self._shell_quote(pChapSecret) + "\""
    ...
    def RefreshTargets(self, pPortalAddress, pExpectedTargetCount=0):
        if self.RemoteOs == OsType.Windows:
            ...
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --refresh_targets")
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
    ...
    def _parse_iscsiapp_error(self, pStdout):
    ...
    def LoginTargets(self, pPortalAddress, pLoginOrder = "serial", pTargetList = None):
        if self.RemoteOs == OsType.Windows:
            ...
            cmd = "iscsiapp.exe --login_targets --portal_address=" + pPortalAddress + " --chap_user=\"" + self._shell_quote(chap_user) + "\" --chap_secret=\"" + self._shell_quote(chap_secret) + "\""
            #retcode, stdout, stderr = self.ExecuteCommand(cmd)
    ...
    def LogoutTargets(self, pTargetList=None):
        ...
        if self.RemoteOs == OsType.Windows:
            if pTargetList != None and len(pTargetList) > 0:
                raise ClientError("target_list is not implemented for Windows")
            retcode, stdout, stdin = self.ExecuteCommand("iscsiapp.exe --logout_targets --force_unmount --persistent")
            if retcode == 0:
    ...
    def GetAllTargets(self):
        if self.RemoteOs == OsType.Windows:
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp --list_targets")
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
            targets = []
    ...
    def GetLoggedInTargets(self):
        if self.RemoteOs == OsType.Windows:
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --list_targets")
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
            targets = []
    ...
    def GetVdbenchDevices(self):
        if self.RemoteOs == OsType.Windows:
           # Give this a longer than default timeout because it can take a while when there a large number of disks
            retcode, stdout, stderr = self._execute_winexe_command(self.IpAddress, self.Username, self.Password, "iscsiapp.exe --list_disks", 180)
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
            devices = []
    ...
    def GetVolumeSummary(self):
        if self.RemoteOs == OsType.Windows:
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --list_disks")
            if retcode != 0:
                self._debug(stdout)
                raise ClientError(self._parse_iscsiapp_error(stdout))
            line_list = []
    ...
    def SetupVolumes(self):
        if self.RemoteOs == OsType.Windows:
            retcode, stdout, stderr = self.ExecuteCommand("iscsiapp.exe --setup_disks --force_mountpoints --relabel")
            if retcode == 0:
