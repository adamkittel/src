...
#     client.connect('192.168.135.20',
#                    username='root',
#                    password='solidfire')

#     scp = scp_client.SCPClient(client.get_transport())
#     scp.get('/root/70-persistent-net.rules')
...
class SCPClient(object):
    ...
    def _recv_file(self, cmd):
        chan = self.channel
        parts = cmd.split()
        print "_recv_file: parts - ", parts
        try:
            mode = int(parts[0], 8)
            size = int(parts[1])
            path = os.path.join(self._recv_dir, parts[2].replace(':', '_'))
        except:
            chan.send('\x01')
            chan.close()
